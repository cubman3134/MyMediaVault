...
**Date:** 2026-07-21
**Status:** Approved design; plan queues after the sync-controls track merges.
...
- Per (profile, item): `favorite` (bool), `hidden` (bool), `completion`
  (None | InProgress | Finished | Abandoned | Planned), `tags` (QStringList).
- Item keys: the SAME stable keys the app already uses (addon itemId / local path
  keys); hashed internally before use as ini group paths (the SyncOffsets lesson —
  the store owns key sanitization; MD5-over-UTF8 hex tokens).
- Storage: `marks/<profileId>/<itemHash>/...` + per-profile tag vocabulary
  `marks/<profileId>/tagVocab`. Empty-key = no-op, no junk keys. Removing a tag from
  the vocabulary strips it from all items (per profile).
...
- Probe (`probe_marks`): store semantics — per-profile isolation (profile A's marks
  ...
  hidden/favorite/completion round-trips, cache invalidation on change + profile
